...
function ModalOpenPack() {
  ...
  const [diffemonContract, setDiffemonContract] =
  ...
  const diffemonAddress =
    connectedChain === "polygon" ? DiffemonPolygon : DiffemonRinkeby;
  ...
  useEffect(() => {
    const loadContract = async () => {
      if (typeof window !== "undefined") {
        ...
        const ethprovider: ethers.providers.Web3Provider =
          new ethers.providers.Web3Provider((window as any).ethereum);
        await ethprovider.send("eth_requestAccounts", []);
        setEthersProvider(ethprovider);
        const signer = new ethers.Wallet(
          process.env.NEXT_PUBLIC_PACK_ADMIN_PRIVATE_KEY,
          provider
        );
        const diffemonContract: ethers.Contract = new ethers.Contract(
          diffemonAddress,
          DiffemonABI,
          signer
        );
  ...
  const allotBoosters = async () => {
    ...
    const allotPackets = async () => {
        if (DiffemonBoosterCodes.include(code.toLowerCase())) {
            if (!session) {
  ...
  const getCards = async () => {
    ...
    const diffemonContractMetamask: ethers.Contract = new ethers.Contract(
      diffemonAddress,
      DiffemonABI,
      signer
    );
    try {
      setIsAllotted(false);
      await diffemonContractMetamask?.buyBooster({
        gasPrice: feeData.gasPrice,
      });
    ...
    <>
      ...
      <div className="flex flex-col space-y-3 justify-center items-center">
        <input
          ...
          className="p-5 h-8 placeholder:text-base-100 placeholder:italic rounded-md bg-transparent border shadow-lg outline-none"
        ...
        {isAllotted && (
          <button className="btn btn-primary" onClick={() => getCards()}>
            Mint your cards
          </button>
        )}
      </div>
